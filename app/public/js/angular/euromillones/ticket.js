...
function Controller($scope, $http, VariosService, $window, euromillones) {
    ...
    $scope.determinarNumeroAciertos = function(combinacion){
        return euromillones.getSuccessfulNumbersAmount($scope.ticket.resultado, combinacion);
    }

    $scope.determinarCategoriaPremio = function (combinacion) {
        return euromillones.getPrizeCategory($scope.ticket.resultado, combinacion);
    };
        ...
        euromillones.getTickets({
            ...
            .then(function(data){
                ...
                if(tickets.length > 0 && tickets[0].sorteo){
                    $window.location.href = "/euromillones/tickets/" + tickets[0].anyo + "/" + tickets[0].sorteo;
                }else{
        ...
        if(nuevoSorteo >= 1) {
            ...
            euromillones.getTickets({
                ...
                .then(function(data) {
                    tickets = data.data;
                    if(tickets.length > 0 && tickets[0].sorteo) {
                        $window.location.href = "/euromillones/tickets/" + tickets[0].anyo + "/" + nuevoSorteo;
                    }else{
